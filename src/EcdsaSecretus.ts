...
class EcdsaSecretus {
    private Ln: bigint = 0n;
    private keyGEN: Keygen = new Keygen();
    private myEcc: Ecc = new Ecc();

    private point: [bigint, bigint] = [0n, 0n];
    private pointNULL: [bigint, bigint] = [0n, 0n];

    constructor() {
        let n_order: bigint = this.myEcc.n_order;
        while (n_order > 0n) {
    ...
    ///////////////////////////////////////////////////////////
    public Knum(HA4: string, PVTKEY: string, e: string): bigint {
        //Verificar os NULLs
        let x: string | null = SHA256G.SHA256MsnHxHex2(SHA256G.HashStrToByte2(PVTKEY + e), SHA256G.HashStrToByte2(HA4));
        while (x === null) {
            x = x + "1";
            x = SHA256G.SHA256MsnHxHex2(SHA256G.HashStrToByte2(PVTKEY + e + x), SHA256G.HashStrToByte2(HA4));
        }

        let H1a: string | null = SHA256G.SHA256MsnHxHex2(SHA256G.HashStrToByte2(e), SHA256G.HashStrToByte2(x));
        while (H1a === null) {
            H1a = H1a + "1";
            H1a = SHA256G.SHA256MsnHxHex2(SHA256G.HashStrToByte2(e + H1a), SHA256G.HashStrToByte2(x));
        }

        let H1: bigint = this.keyGEN.HashToBigInt(H1a);
    ...
    ///////////////////////////////////////////////////////////
    ECDSABSV (e: string, PVTKEY: string, HA4: string): [bigint, bigint, bigint] {
        let signECDSA: [bigint, bigint, bigint] = [0n, 0n, 0n];
        ...
        */
        ...
        while (Flag == false) {
            ...
            if( r == 0n ) {
                ...
                /////////////////////////////////////////////////////
                // (K^-1) mod n
                let kinv: bigint = this.myEcc.inverse(K, this.myEcc.n_order);
              
                // (r * PVTKEY) mod n
                let rdA: bigint = r * this.myEcc.modp(this.keyGEN.HashToBigInt(PVTKEY), this.myEcc.n_order);
    ...
    ///////////////////////////////////////////////////////////
    public ECDSAVerifyBSV2(e: string, pubKey: bigint[], sign: bigint[]): number {
        let signECDSA: bigint[] = [0n, 0n];
        /////////////////////////////////////////////////////////////////////////////////////////////
        ...
        */
        ...
        let z: bigint = this.keyGEN.HashToBigInt(e);
        ...
        // Passo 4
        let sInv: bigint = this.myEcc.inverse(sign[1], this.myEcc.n_order);

        let u1: bigint = z * sInv;
        let u2: bigint = sign[0] * sInv;
        u1 = this.myEcc.modp(u1, this.myEcc.n_order);
        u2 = this.myEcc.modp(u2, this.myEcc.n_order);

        // Passo 5
        let P1: [bigint, bigint] = [0n, 0n];
        let P2: [bigint, bigint] = [0n, 0n];
        let C: [bigint, bigint] = [0n, 0n];
    ...
    private eccNPoint(n: bigint, x: bigint, y: bigint): [bigint, bigint] {
