...
class Keygen {
    ...
    publicKey(text) {
        ...
        let cont = 1n;
        let secKey;
        let pubKey64 = new Array(44);
        secKey = text.split('');
        if (text.length <= 31)
            keySize = text.length % 32;
        else
            keySize = 32;
        ...
        if (point[1] & 1n)
            pubKey64[0] = this.base64[3];
        else
            pubKey64[0] = this.base64[2];
        text = pubKey64.join('');
        return text;
    }
    /////////////////////////////////////////////////
    ...
    pubKeyRev(pubkey) {
        ...
        for (let i = pubkey.length - 1; i > pubkey.length - 44; i--) {
            let x;
            for (x = 0; x < 64; x++)
                if (this.base64[x] === pubKey1[i])
                    break;
            point[0] = point[0] + tA * BigInt(x);
            tA = tA * 64n;
        }
        point[1] = this.sqrtCF.sqrtCF(this.keys.modp(point[0] * this.keys.modp(point[0] * point[0], this.keys.p) +
            this.keys.A * point[0] +
            this.keys.B, this.keys.p), this.keys.p);
        let pubKey2 = SHA256G.StrToByte(pubkey);
        if ((point[1] % 2n === 0n && (pubKey2[0] & 0x01) === 1) ||
    ...
    pubKeyCompSECRev(pubkey) {
        ...
        point[1] = this.sqrtCF.sqrtCF(this.keys.modp(point[0] * this.keys.modp(point[0] * point[0], this.keys.p) +
            this.keys.A * point[0] +
            this.keys.B, this.keys.p), this.keys.p);
        if ((point[1] % 2n === 0n && (pubkey.charAt(0) === '3')) ||
    ...
    //////////////////////////////////////////////////////////////////
    pubKeyUncompSECRev(pubkey) {
        let point = [0n, 0n];
        let tA = 1n;
        pubkey = pubkey.substring(1);
        if (pubkey.length !== 65 + 64)
            return point;
        //console.log('Aqui', pubKey1.length);
        for (let i = pubkey.length - 1; i > ((pubkey.length - 1) - 64); i--) {
            let x;
    ...
    /////////////////////////////////////////////////////////////
    static bItoHexStr(bInumber) {
        let cSHA = [100]; // chave comprimida
        let ichA = bInumber;
        //for (let i = 99; i > 0; i--) {
        for (let i = 99; i >= 0; i--) {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////        
    bsvWalletFull(pubkeyCOD, Compressed) {
        let pubKey = this.pubKeyRev(pubkeyCOD);
        let chA = new Array(64);
        let cSHA = new Array(33); // compressed key
        let cSHA2 = new Array(65); // uncompressed key
        let tA, ichA, ichA2;
        ////////////////////////////////////////////////////////////////////
        ...
        ////////////////////////////////////////////////////////////////////
        let pRIPEMD160 = new Array(21);
        let pRIPEMD1602 = new Array(21);
        let RIPEMDoutj = new Array(RIPEMDout.length);
        let RIPEMDout2j = new Array(RIPEMDout2.length);
        ...
        let VPC = 0n, VPCcont = 1n, VPC2 = 0n, VPCcont2 = 1n;
        let SHA256outj = new Array(Math.floor(SHA256out.length / 2));
        let SHA256out2j = new Array(Math.floor(SHA256out2.length / 2));
    ...
    addRMD(hashKey) {
        ...
        let addrINT = 0n;
        let x;
        const base58begin = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz".split('');
    ...
    //  publicKeyHEX(text: string): string
    //////////////////////////////////////////////////////////////////////////////////        
    bsvWalletRMD160(pubkeyCOD, Compressed) {
        let pubKey = this.pubKeyRev(pubkeyCOD);
        let chA = new Array(64);
        let cSHA = new Array(33); // compressed key
        let cSHA2 = new Array(65); // uncompressed key
    ...
    sigDERrev(signECDSA) {
        ...
        let x = parseInt(signECDSA.substring(0, 2), 16);
        let r = signECDSA.substring(2, 2 * x + 2);
        signECDSA = signECDSA.substring(2 * x + 2);
        let s = signECDSA.substring(4);
        let bISign = [BigInt(0), BigInt(0)];
        let cR = SHA256G.HashStrToByte2(r);
        let cS = SHA256G.HashStrToByte2(s);
    ...
    //////////////////////////////////////////////////////////////////////////////////        
    sigDER(signECDSA) {
        const cSHA = new Array(100); // chave comprimida
        const cSHA2 = new Array(100); // chave não comprimida
        let tA, ichA, ichA2;
        ichA = signECDSA[0];
        ichA2 = signECDSA[1];
        for (let i = 99; i > 0; i--) {
        ...
        let s = SHA256G.ByteToStrHex(cSHA2);
        let is = 0;
        ...
        let jr = cSHA[ir] & 0xff;
        let js = cSHA2[is] & 0xff;
        if (jr > 0x7f)
            r = "00" + r;
        if (js > 0x7f)
            s = "00" + s;
        let sizeR = new Array(1); // chave comprimida
        let sizeS = new Array(1); // chave não comprimida
        let sizeSig = new Array(1); // chave não comprimida
